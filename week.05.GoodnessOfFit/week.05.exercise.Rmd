...
x <- rchisq(84, 9)
chi_curve = dchisq(x, df= 9)

ggplot() + geom_density(mapping = aes(x = observed_p), color = 'red') + 
        geom_line(mapping = aes(x = x, y = chi_curve))
```

We expect the chi2 dist with 9 degrees of freedom to fit the p values under the h0

>Q4. Is the distribution of empirical p-values compatible with H0?

Not really, as seen from the above graph
...
P_100misses = P1 * P2 * P3 * .... * P100. The probability is basically just the product of the individual probabilities. In order to have 100 misses you need cell1 to miss and cell2 to miss and cell3 to miss and ... and cell100 to miss.
...
i_samples = rpois(i_len, 5)
vl_samples = rpois(vl_len, 1)
geom_samples = vector('double', length = 5000)
geom_samples[1:i_len] = i_samples
geom_samples[(i_len + 1):5000] = vl_samples

ggplot() + geom_density(mapping = aes(geom_samples),  color = 'green') +
  geom_density(mapping = aes(rpois(5000, 5)))
...
Yes, as it can be seen from the above graph.
...
```{r}
data_sets = vector('list')

for (i in 1:1000) {
  data_set = vector('double')
  for (x in c(5, 10, 20, 25, 50)) {
    nof_ind = 500 / x
    sim = rpois(nof_ind, x)
    data_set = append(data_set, sim)
  }
