...
ggplot() + geom_line(mapping = aes(x = 1:35, y = expected_cp)) +
   geom_point(mapping = aes(x = 1:35, y = observed_cp), color = 'green')
```
...
```
end_pos = median(data_3L$pos)

f <- function(df, pos) {
  #print(df)
  
  tt <- df[1:1000,] %>% 
    filter(chr == '3L' & pos < end_pos) %>%
...
r2_summ = snps_3L %>%
  group_by(pos) %>%
  summarise(
    p0 = length(genotype[genotype == '2'])/length(genotype),
    p2 = length(genotype2[genotype2 == '2'])/length(genotype2),
    p02 = length(haplotype[haplotype == '2 2'])/length(haplotype),
...
r2_vs_dist = r2_summ %>%
  mutate(bpg = 50 + 100 * bp %/% 100) %>%
  group_by(bpg) %>%
  summarise(r2g = mean(r2, na.rm = TRUE))

ggplot(r2_vs_dist, aes(x = log10(bpg), y = r2g)) + geom_line()
...
Running the computations on all the 3L SNP variants that appea on lower positions in the chromozome, we see similar results as the ones obtained in the paper. The LD has a logarithmic decrease with respect to the physical distances between variants.
