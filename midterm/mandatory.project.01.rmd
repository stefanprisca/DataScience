...
```{r}
end_pos = median(data_3L_varc$pos)
snps_3L = (data_3L_varc %>% filter(pos < end_pos & var_class == 'SNP'))[1:20000,]
...
snps_3L_g['genotype2'] = lead(snps_3L_g$genotype, 1)
snps_3L_g = snps_3L_g %>% mutate(haplotype = paste(genotype, genotype2, sep = ' '))
```
...
r2_summ = snps_3L_g %>%
  group_by(pos) %>%
  summarise(
    p0 = length(genotype[genotype == '0'])/length(line),
    p2 = length(genotype2[genotype2 == '2'])/length(line),
    p02 = length(haplotype[haplotype == '0 2'])/length(line),
...
r2_vs_dist = r2_summ %>%
  group_by(bp) %>%
  filter(bp < 1000) %>% # there is a 5000 distance in my data, which makes a mess of the graph....
  summarise(bpg = mean(5 + 10 * bp %/% 10), r2g = mean(r2, na.rm = TRUE)) %>%
  group_by(bpg) %>%
  summarise(r2g = mean(r2g, na.rm = TRUE))

ggplot(r2_vs_dist, aes(x = bpg, y = r2g)) + geom_line()
```
