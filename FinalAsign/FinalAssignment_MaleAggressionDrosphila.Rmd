...
point_plot = ggplot(pheno_ord) + geom_point(mapping= aes(lineid_n, aggression_score)) + ggtitle('Aggression Scores per Line')
  
box_plot = ggplot(pheno_ord) + geom_boxplot(mapping = aes(bin_id, aggression_score)) + ggtitle('Aggression Scores Boxplot')

multiplot(point_plot, box_plot)
...
effect_size_boot = rep(NA, total_size)
...
for (i in 0:nof_bootstraps){
  ...
  sample_ = base_df[rows,] %>%
    summarise(mean_geno = mean(Aggression_score)) %>%
    group_by(id) %>%
    spread(genotype, mean_geno)
  
  
  sample_effect_size = as.list((sample_['2'] - sample_['0'] ) / 2)[[1]]
  
  effect_size_boot[seq(i*4 + 1, i*4+4, 1)] = sample_effect_size
...
write_lines(effect_size_boot, "../datasets/effect_size_boot")


ggplot() + geom_density(mapping = aes(effect_size_boot))
SE = sd(effect_size_boot) / sqrt(total_size)
