...
mamm_summ = mammals %>%
  ...
  summarise(med_RPKM = median(RPKM), med_dnds = median(dNdS), nof_genes = n())
...
Residual standard error: 0.05761 on 22 degrees of freedom
...
```{r}
chimp_residuals = chimp_data
chimp_residuals['residual'] = fit_chimp$residuals

ggplot(chimp_residuals) + geom_point(mapping = aes(med_dnds, residual, size = nof_genes), shape = 'O') +
  geom_hline(mapping = aes(yintercept = 0), color = 'green', size = 2, linetype = 2) +
  geom_smooth(method='lm', mapping = aes(med_dnds, residual), color ='red', se = FALSE)
```

Well, it does look good considering the regression line. The residual values are as they were expected. But it seems like data is moving away from the regression line. The the trend is for residuals to grow as dnds grows. This can be seen by the second regression line drawn on residuals. This is due to the outlier, which could be removed by reshaping the data.
...
```{r}
fit_chimp_w = lm(data = chimp_data, med_dnds ~ med_RPKM, weights = nof_genes)
